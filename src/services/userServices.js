...
const editUser = async (userId, userData) => {
	try {
		...
		if (userData.username) {
			const userCheck = await UserModel.findOne({ username: userData.username });
...
const resetPassword = async (userId, userData) => {
	...
	try {
		const isPasswordCorrect = await cryptoHelper.comparePassword(userData.oldPassword, user.password);
